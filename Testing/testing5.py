...
class xiaMiSpider(unittest.TestCase):
    ...
    def saveBrief(self,content,name):
        fileName =  self.fileURL +  "/" +name + ".txt"
        #f = codecs.open(fileName,'w+','utf-8')
        f = open(fileName,'w+')
        print u"正在保存信息为",fileName
        #f.write(str(content).encode('utf-8'))
        f.write(str(content).decode('unicode-escape'))
    ...
    def savePageInfo(self,pageIndex):
        ...
        for item in contents:
            # item[0]是歌曲链接，item[1]歌曲名称，item[3]是艺人，item[4]专辑
            print u"艺人",item[2],u"的专辑",item[3],u",中的歌曲",item[1]
            self.saveBrief(item,item[1])
